        ...
        while not finished:
            ...
            for rec in raw_res['query']['categorymembers']:
                if rec['type'] == 'page':
                    ...
                    subcats.append(tmp)
            print(raw_res)
            # MODIFIED FOR PRACTICALPLANTS.ORG
            if 'query-continue' not in raw_res or last_cont == raw_res['query']['categorymembers']:
                print("last_cont", last_cont)
                break
            returned_results = returned_results + current_pull
            print("results:", returned_results, current_pull)
            if results is None or (results - returned_results > 0):
                # MODIFIED FOR PRACTICALPLANTS.ORG
                last_cont = raw_res['query-continue']['categorymembers']
            else:
                finished = True
        # end while loop
